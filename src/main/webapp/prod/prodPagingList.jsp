...
<%-- 				<%} %> --%>
				<c:forEach items="${prodpageList }" var="prod_vo" >
					<tr class="prodListClick">
						...
						<td><fmt:formatDate value = "${prod_vo.prod_insdate }" pattern="yyyy년 MM월 dd일"></fmt:formatDate></td>
					</tr>
...
<!--     			</li> -->
				...
				<c:set var="pageCnt" value="${prodpageCnt }"/>
				<c:forEach begin="1" end="${prodpageCnt }" var="p">
				
					<li><a href="/prodList?page=${p }&pageSize=10">${p }</a></li>
				</c:forEach>
<%-- 				<%} %> --%>
				<li>
      				<a href="/prodList?page=${prodpageCnt }&pageSize=10" aria-label="Next">
       					 <span aria-hidden="true">&raquo;</span>
     				 </a>
    			</li>
			</ul>
